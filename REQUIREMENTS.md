...
- [] UI
  - [] 주문확인
  - [] 쿠폰선택 모달
    - [] 쿠폰이 두 개 선택되었다면 나머지 쿠폰 비활성화
  - [] 결제 확인

- 사용자 흐름 기반
  - [] 사용자가 장바구니 페이지에서 주문 확인 버튼을 클릭했다.
    - [] 임시 주문서 생성하기.(API POST /api/orders/)
      - [] 임시 주문서에서 가진 쿠폰을 통해 최대효율 쿠폰 판단하고 적용하기.
      - [] 제주/도서 산간은 기본으로 False로 하기.

  - [] 사용자가 주문 확인 페이지에 도달했다.
    - [] 임시 주문서 가져와서 데이터 보여주기.(API GET /api/orders/{orderId}/)

  - [] 사용자가 쿠폰 적용 버튼(주문 확인 페이지에서)을 클릭했다.
...
- [] 쿠폰
  ...
  - [] 검증
    - [] 최대 2개 쿠폰 사용(수량제한)
    - [] 만료일
  - [] 최대효율 쿠폰 판단(정액 쿠폰 우선하여 계산, 제주도 도서산간 여부 포함)

- [] 배송비
  - [] 무료배송(100000 이상)
  - [] 제주도 도서산간시 배송비 3000원 추가
